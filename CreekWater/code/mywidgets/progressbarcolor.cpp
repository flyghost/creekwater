...
void ProgressBarColor::drawBar(QPainter *painter)
{
    ...
    //自动计算文字字体大小
    QFont font;
    font.setPixelSize(static_cast<int>((width() / 10) * 0.35));
    painter->setFont(font);

    //计算进度值字符的宽度
    double currentValue = static_cast<double>(value - minValue) * 100 / (maxValue - minValue);
    ...
    painter->drawText(textRect, Qt::AlignCenter, text + strValue);
    ...
    int initX = 5;
    int lineCount = static_cast<int>(barBgRect.width() / step) ;
    double lineX = static_cast<double>(barBgRect.width()) / lineCount;
...
void ProgressBarColor::setBarText(QString str)
{
    this->text = str;
}
